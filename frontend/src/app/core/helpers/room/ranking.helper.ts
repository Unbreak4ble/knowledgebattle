...
export function toRankingList(live_questions:any, correct_questions:any){
    ...
    (live_questions?.map((x:any) => x.players.map((y:any) => ({[y.player_id]:{count: 0, name: y.player_name || '[ghost]'}}))) || []).forEach((x:any)=>{
        x.forEach((y:any)=>{
    ...
    for(const question of live_questions){
        ...
        for(const player of players){
            ...
            ++correct_pickers[player.player_id].count;
        }
    }
    ...
    const sort = () =>{
        for(const picker in correct_pickers){
            ...
            for(const also_picker in correct_pickers){
                if(correct_pickers[picker].count >= correct_pickers[also_picker].count) ++count;
            }

            if(count >= pickers_count){
                top.push({id: picker, ...correct_pickers[picker]});
